...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (i = 0; i < 3; i++) {
            if ((grid[i][j] == 'o') && (grid[i][j] == grid[i][j + 1]) && (grid[i][j + 1] == grid[i][j + 2])) {
                result = "O wins";   /*jesli ktoras kolumna to 'o' zakoncz petle*/
                break;
            } else {
                if ((grid[i][j] == 'x') && (grid[i][j] == grid[i][j + 1]) && (grid[i][j + 1] == grid[i][j + 2])) {
                    result = "X wins";   /*jesli ktoras kolumna to 'x' zakoncz petle*/
                    break;
                }
            }
        }
        ...
        for (j = 0; j < 3; j++) {
            i=0;
            if ((grid[i][j] == 'o') && (grid[i][j] == grid[i + 1][j]) && (grid[i + 1][j] == grid[i + 2][j])) {
                result = "O wins";
                break;
            } else {
                if ((grid[i][j] == 'x') && (grid[i][j] == grid[i + 1][j]) && (grid[i + 1][j] == grid[i + 2][j])) {
                    result = "X wins";
                    break;
                }
            }
        ...
        //}
        //if (result == "None") {
        if ((grid[1][1] == 'o') && ((grid[1][1] == grid[0][0] && grid[1][1] == grid[2][2]) || (grid[1][1] == grid[0][2] && grid[1][1] == grid[2][0]))) {
            result = "O wins";
        } else  {
            if ((grid[1][1] == 'x') && ((grid[1][1] == grid[0][0] && grid[1][1] == grid[2][2]) || (grid[1][1] == grid[0][2] && grid[1][1] == grid[2][0]))) {
                result = "X wins";
            }
        }
        //}
